...
章节来源
- [web/src/router/index.js:9-47](file://web/src/router/index.js#L9-L47)
- [web/src/main.js:19-19](file://web/src/main.js#L19-L19)
- [web/src/App.vue:2-6](file://web/src/App.vue#L2-L6)
- [web/src/layout/MainLayout.vue:24-34](file://web/src/layout/MainLayout.vue#L24-L34)
...
- [web/src/App.vue:1-28](file://web/src/App.vue#L1-L28)
- [web/src/layout/MainLayout.vue:1-228](file://web/src/layout/MainLayout.vue#L1-L228)
- [web/src/views/ScriptList.vue:268-505](file://web/src/views/ScriptList.vue#L268-L505)
- [web/src/views/ScriptEdit.vue:285-505](file://web/src/views/ScriptEdit.vue#L285-L505)
...
### 路由懒加载与性能优化策略
- 当前实现
  - 路由组件为同步导入（import 直接在文件顶部声明）
- 懒加载建议
  - 将路由组件改为异步导入（动态 import），减少首屏包体积
  - 结合 Vite 的代码分割能力，按需加载页面组件
- 其他优化
  - 使用 keep-alive 缓存不常变动的页面
  - 对大型页面（如编辑器）在进入时再初始化重型实例

章节来源
- [web/src/router/index.js:2-7](file://web/src/router/index.js#L2-L7)
...
- [web/src/views/ScriptEdit.vue:287-287](file://web/src/views/ScriptEdit.vue#L287-L287)
...
## 性能考虑
- 代码分割与懒加载
  - 将路由组件改为动态 import，利用 Vite 的按需加载能力
- 资源与网络
...
## 结论
本项目采用标准的 Vue Router 4 + 布局 + 子页面的路由结构，配合 Axios 的统一拦截与后端 Gin 的 API 路由，实现了清晰的前后端分离架构。建议后续引入路由守卫完善权限控制，采用动态导入实现懒加载，并在全局后置守卫中完成页面标题与错误处理的统一管理，以进一步提升安全性与用户体验。