# 执行流程控制

<cite>
**本文引用的文件**
- [execution.go](file://internal/service/execution.go)
- [execution_handler.go](file://internal/handler/execution.go)
- [execution_model.go](file://internal/model/execution.go)
- [slave_service.go](file://internal/service/slave.go)
- [slave_model.go](file://internal/model/slave.go)
- [script_service.go](file://internal/service/script.go)
- [config.go](file://config/config.go)
- [db.go](file://internal/database/db.go)
- [response_model.go](file://internal/model/response.go)
- [csv_split.go](file://internal/service/csv_split.go)
</cite>

## 更新摘要
**变更内容**
- 新增失败请求明细捕获功能，支持本地和分布式环境的HTTP请求/响应详情记录
- 增强Master参与执行机制，includeMaster参数控制Master是否参与分布式执行
- 新增CSV数据分片功能，splitCSV参数控制CSV文件的自动拆分和Slave节点分发
- 扩展执行选项参数，增加SaveHTTPDetails、IncludeMaster、SplitCSV三个新选项
- 增强错误处理和容错机制，支持错误明细的分布式上传和存储

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考虑](#性能考虑)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件聚焦"执行流程控制"的完整生命周期，覆盖从脚本验证、Slave节点选择、JMeter命令构建、异步执行启动，到分布式与本地模式切换、includeMaster参数机制、JVM内存参数动态计算、错误处理与容错、状态原子性更新与并发安全、以及执行时序与关键决策点。特别关注新增的失败请求明细捕获、Master参与执行和CSV数据分片等增强功能。目标是帮助开发者与运维人员全面理解系统在真实生产环境中的行为与优化方向。

## 项目结构
该系统采用分层设计：Handler负责HTTP接口与参数绑定；Service封装业务流程与执行控制；Model定义数据模型；Database封装SQLite初始化与迁移；Config提供全局配置；Web前端通过API消费后端能力。

```mermaid
graph TB
subgraph "接口层"
H1["execution_handler.go<br/>REST接口"]
end
subgraph "服务层"
S1["execution.go<br/>执行控制与JVM内存计算"]
S2["slave_service.go<br/>Slave心跳与连通性"]
S3["script_service.go<br/>脚本与JMX文件管理"]
S4["csv_split.go<br/>CSV文件拆分与分发"]
end
subgraph "数据层"
D1["db.go<br/>SQLite初始化与迁移"]
M1["execution_model.go<br/>执行记录模型"]
M2["slave_model.go<br/>Slave节点模型"]
end
subgraph "配置与静态"
C1["config.go<br/>全局配置"]
end
H1 --> S1
H1 --> S2
H1 --> S3
S1 --> D1
S2 --> D1
S3 --> D1
S4 --> D1
S1 --> M1
S2 --> M2
S1 --> C1
```

**图表来源**
- [execution_handler.go:39-53](file://internal/handler/execution.go#L39-L53)
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [slave_service.go:16-41](file://internal/service/slave.go#L16-L41)
- [script_service.go:18-83](file://internal/service/script.go#L18-L83)
- [db.go:15-34](file://internal/database/db.go#L15-L34)
- [config.go:10-41](file://config/config.go#L10-L41)
- [csv_split.go:1-145](file://internal/service/csv_split.go#L1-L145)

**章节来源**
- [execution_handler.go:39-53](file://internal/handler/execution.go#L39-L53)
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [slave_service.go:16-41](file://internal/service/slave.go#L16-L41)
- [script_service.go:18-83](file://internal/service/script.go#L18-L83)
- [db.go:15-34](file://internal/database/db.go#L15-L34)
- [config.go:10-41](file://config/config.go#L10-L41)
- [csv_split.go:1-145](file://internal/service/csv_split.go#L1-L145)

## 核心组件
- **执行服务（CreateExecution）**：负责脚本校验、Slave节点过滤与在线状态确认、执行记录创建、结果目录与路径写入、JMeter命令构建（本地/远程/混合）、JVM内存参数注入、异步执行与日志落盘、最终状态更新与摘要生成。**新增**：支持失败请求明细捕获、Master参与执行、CSV数据分片等功能。
- **Handler（CreateExecution）**：REST接口入口，参数绑定与错误返回，支持SaveHTTPDetails、IncludeMaster、SplitCSV三个新参数。
- **Slave服务**：心跳检测、连通性检查、批量状态更新。
- **Script服务**：脚本列表、JMX文件读写与校验。
- **CSV拆分服务**：将大型CSV文件均匀拆分为多个分片，支持分布式执行的数据隔离。
- **数据库**：SQLite初始化、表结构与索引、迁移。
- **配置**：Server、JMeter、Slave、Dirs等全局配置。

**章节来源**
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [execution_handler.go:39-53](file://internal/handler/execution.go#L39-L53)
- [slave_service.go:112-157](file://internal/service/slave.go#L112-L157)
- [script_service.go:118-134](file://internal/service/script.go#L118-L134)
- [db.go:36-124](file://internal/database/db.go#L36-L124)
- [config.go:10-41](file://config/config.go#L10-L41)
- [csv_split.go:1-145](file://internal/service/csv_split.go#L1-L145)

## 架构总览
下图展示了从HTTP请求到执行完成的端到端流程，包括分布式与本地模式切换、includeMaster参数作用、JVM内存动态计算、异步执行与状态更新，以及新增的失败请求明细捕获和CSV数据分片功能。

```mermaid
sequenceDiagram
participant Client as "客户端"
participant Handler as "执行Handler"
participant Service as "执行Service"
participant DB as "数据库"
participant Proc as "进程组(sync.Map)"
participant JMeter as "JMeter进程"
Client->>Handler : POST /api/executions<br/>SaveHTTPDetails, IncludeMaster, SplitCSV
Handler->>Service : CreateExecution(scriptID, slaveIDs, remarks, saveHTTPDetails, includeMaster, splitCSV)
Service->>DB : 查询脚本与Slave在线状态
DB-->>Service : 脚本信息/在线Slave主机列表
Service->>Service : 生成执行记录与结果目录
Service->>Service : 动态计算JVM内存参数
Service->>Service : 检查CSV分片需求
Service->>Service : 拆分CSV文件并分发到Slave
Service->>Service : 构建本地/远程/JMeter命令
Service->>Proc : 异步启动goroutine并注册进程组
Proc-->>Service : 并发执行本地与远程命令
Service->>JMeter : 启动JMeter进程(带JVM参数)
JMeter-->>Service : 写入日志与结果文件
Service->>Service : 合并JTL/生成报告/解析摘要
Service->>Service : 处理失败请求明细捕获
Service->>DB : 更新执行状态/时长/摘要
Service-->>Handler : 返回执行ID与状态
Handler-->>Client : 200 OK
```

**图表来源**
- [execution_handler.go:39-53](file://internal/handler/execution.go#L39-L53)
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [execution.go:368-463](file://internal/service/execution.go#L368-L463)
- [db.go:36-124](file://internal/database/db.go#L36-L124)

## 详细组件分析

### 执行生命周期与关键决策点
- **脚本验证与路径解析**：查询脚本记录，确保存在且文件路径有效。
- **Slave节点选择与在线状态**：按ID查询并过滤status=online的节点，记录离线ID集合以便后续提示。
- **includeMaster参数机制**：当启用分布式且includeMaster=true时，本地Master也会参与执行，分别生成result-local.jtl与result-remote.jtl，随后合并为最终result.jtl并生成HTML报告。
- **失败请求明细捕获机制**：
  - 本地模式：通过错误明细监听器记录失败请求的HTTP详情到本地文件
  - 分布式模式：通过上传线程组在执行结束后将错误明细文件上传到Master
  - 支持请求头、响应头、请求体、响应体等完整HTTP信息
- **CSV数据分片机制**：
  - 自动检测JMX中的CSVDataSet节点
  - 将大型CSV文件均匀拆分为多个分片
  - 按Slave数量和includeMaster配置确定分片数量
  - 将分片文件分发到各个Slave节点
  - 修改JMX文件中的CSV路径为Slave上的分片路径
- **命令构建策略**：
  - 仅远程：-R指定Slave主机列表，-G传递全局属性。
  - 仅本地：-n -t -l -q -e -o。
  - 本地+远程：本地写入result-local.jtl，远程写入result-remote.jtl，结束后合并并生成报告。
- **JVM内存参数动态计算**：基于系统可用内存（优先读取/proc/meminfo，回退runtime内存），取80%作为最大堆，最小512MB、最大32GB；初始堆为最大堆的1/4，不低于256MB；可通过环境变量覆盖。
- **异步执行与日志**：使用goroutine并发启动本地与远程命令，统一写入execution.log；合并JTL并生成报告；解析JTL生成摘要；最终原子性更新执行状态、结束时间、时长与摘要。
- **错误处理与容错**：
  - 离线Slave：若仅选择离线节点，直接报错；若部分离线，记录警告并继续使用在线节点。
  - 部分失败降级：本地或远程任一失败，仍尝试合并与生成报告，错误信息汇总返回。
  - 停止执行：通过sync.Map定位进程组，逐一kill进程，更新状态为stopped。
  - 清理陈旧执行：服务启动时将status=running的记录标记为failed，避免僵尸状态。

```mermaid
flowchart TD
Start(["开始"]) --> LoadScript["查询脚本信息"]
LoadScript --> CheckScript{"脚本存在?"}
CheckScript --> |否| FailScript["返回错误: 脚本不存在"]
CheckScript --> |是| LoadSlaves["查询Slave并过滤在线节点"]
LoadSlaves --> HasOnline{"是否存在在线节点?"}
HasOnline --> |否| FailOffline["返回错误: 选中的Slave均不在线"]
HasOnline --> |是| CreateExec["创建执行记录与结果目录"]
CreateExec --> CheckCSV{"需要CSV分片?"}
CheckCSV --> |是| SplitCSV["拆分CSV文件并分发到Slave"]
CheckCSV --> |否| BuildArgs["构建JMeter命令(本地/远程/混合)"]
SplitCSV --> BuildArgs
BuildArgs --> CalcJVM["动态计算JVM内存参数"]
CalcJVM --> AsyncRun["异步启动进程组"]
AsyncRun --> RunLocal["启动本地JMeter"]
AsyncRun --> RunRemote["启动远程JMeter"]
RunLocal --> Merge["合并JTL/生成报告"]
RunRemote --> Merge
Merge --> Parse["解析JTL生成摘要"]
Parse --> HandleErrorDetails["处理失败请求明细"]
HandleErrorDetails --> Update["原子性更新执行状态/时长/摘要"]
Update --> End(["结束"])
```

**图表来源**
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [execution.go:368-463](file://internal/service/execution.go#L368-L463)
- [execution.go:1359-1430](file://internal/service/execution.go#L1359-L1430)
- [execution.go:1062-1150](file://internal/service/execution.go#L1062-L1150)

**章节来源**
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [execution.go:368-463](file://internal/service/execution.go#L368-L463)
- [execution.go:1359-1430](file://internal/service/execution.go#L1359-L1430)
- [execution.go:1062-1150](file://internal/service/execution.go#L1062-L1150)

### includeMaster 参数机制
- 当存在Slave且includeMaster=true时，系统同时启动本地Master执行与远程Slave执行，分别输出result-local.jtl与result-remote.jtl，随后合并为最终结果并生成报告。
- 若仅includeMaster而无Slave，则仅本地执行。
- 此机制允许在分布式环境中，Master也参与负载与数据收集，便于统一分析。

**章节来源**
- [execution.go:208-235](file://internal/service/execution.go#L208-L235)
- [execution.go:340-347](file://internal/service/execution.go#L340-L347)
- [execution.go:422-436](file://internal/service/execution.go#L422-L436)

### 失败请求明细捕获机制
- **本地模式捕获**：通过Groovy监听器自动记录失败请求的完整HTTP详情到本地error-details.ndjson文件
- **分布式模式捕获**：在JMX中注入错误明细监听器和上传线程组，执行结束后将错误明细文件上传到Master
- **数据完整性**：支持请求头、响应头、请求体、响应体、失败消息等完整信息
- **令牌验证**：使用随机生成的上传令牌确保数据安全性
- **回传机制**：通过HTTP POST请求将错误明细文件回传到Master，支持重试机制

**章节来源**
- [execution.go:372-399](file://internal/service/execution.go#L372-L399)
- [execution.go:1963-2082](file://internal/service/execution.go#L1963-L2082)
- [execution.go:2084-2195](file://internal/service/execution.go#L2084-L2195)
- [execution.go:2325-2374](file://internal/service/execution.go#L2325-L2374)

### CSV数据分片机制
- **自动检测**：解析JMX文件，自动识别CSVDataSet节点和相关CSV文件
- **均匀拆分**：使用基数值算法将数据行均匀分配到各个分片，确保负载均衡
- **分发策略**：将分片文件分发到各个Slave节点，Master可选择性接收额外分片
- **路径替换**：修改JMX文件中的CSV路径为Slave上的分片路径，设置shareMode为shareMode.all
- **清理机制**：执行完成后自动清理Slave节点上的分片文件

**章节来源**
- [execution.go:239-370](file://internal/service/execution.go#L239-L370)
- [execution.go:2971-3025](file://internal/service/execution.go#L2971-L3025)
- [csv_split.go:10-145](file://internal/service/csv_split.go#L10-L145)

### JVM内存参数动态计算算法
- Linux优先读取/proc/meminfo中的MemAvailable，单位KB转MB；否则回退Go runtime统计，若小于1GB则假设4GB。
- 最终取系统可用内存的80%，最小512MB、最大32GB；初始堆为最大堆的1/4，不低于256MB。
- 可通过环境变量JVM_ARGS覆盖默认值。
- 计算结果以-Xms与-Xmx形式注入JMeter进程环境变量。

**章节来源**
- [execution.go:54-101](file://internal/service/execution.go#L54-L101)
- [execution.go:362-366](file://internal/service/execution.go#L362-L366)

### 分布式与本地模式切换逻辑
- **仅远程**：-R列出在线Slave主机；-G传递全局属性；-Dserver.rmi.ssl.disable=true；-Djava.rmi.server.hostname=配置项。
- **仅本地**：-n -t -l -q -e -o；granularity=1000；可选开启错误明细采集。
- **本地+远程**：本地写result-local.jtl，远程写result-remote.jtl，合并后生成报告；错误明细上传URL与令牌在远程侧注入。

**章节来源**
- [execution.go:274-335](file://internal/service/execution.go#L274-L335)
- [execution.go:337-347](file://internal/service/execution.go#L337-L347)

### 执行状态原子性更新与并发安全
- **进程管理**：使用sync.Map按执行ID存储进程组，支持StopExecution时快速定位并kill进程。
- **状态更新**：updateExecutionStatus原子性更新状态、结束时间、时长与摘要；通过查询开始时间计算duration。
- **清理陈旧执行**：服务启动时将status=running的记录标记为failed，避免遗留状态。
- **日志与结果**：统一写入execution.log；JTL合并与报告生成在异步goroutine内完成，完成后才更新状态。

```mermaid
classDiagram
class ExecutionProcessGroup {
+Commands []*exec.Cmd
+Cancel func()
}
class ExecutionService {
+CreateExecution(...)
+StopExecution(id)
+updateExecutionStatus(id,status,summary)
+CleanupStaleExecutions()
}
class DB {
+InitDB()
+createTables()
}
ExecutionService --> DB : "读写执行记录"
ExecutionService --> ExecutionProcessGroup : "并发管理"
```

**图表来源**
- [execution.go:47-53](file://internal/service/execution.go#L47-L53)
- [execution.go:949-994](file://internal/service/execution.go#L949-L994)
- [execution.go:484-502](file://internal/service/execution.go#L484-L502)
- [execution.go:1044-1060](file://internal/service/execution.go#L1044-L1060)
- [db.go:15-34](file://internal/database/db.go#L15-L34)

**章节来源**
- [execution.go:47-53](file://internal/service/execution.go#L47-L53)
- [execution.go:949-994](file://internal/service/execution.go#L949-L994)
- [execution.go:484-502](file://internal/service/execution.go#L484-L502)
- [execution.go:1044-1060](file://internal/service/execution.go#L1044-L1060)
- [db.go:15-34](file://internal/database/db.go#L15-L34)

### 错误处理策略与容错机制
- **离线Slave容错**：若仅选择离线节点，直接报错；若部分离线，记录警告并继续使用在线节点。
- **部分失败降级**：本地或远程任一失败，仍尝试合并与生成报告，错误信息汇总返回。
- **停止执行**：通过进程ID定位进程组，逐一kill进程，更新状态为stopped。
- **清理陈旧执行**：服务启动时将status=running的记录标记为failed，避免僵尸状态。
- **结果解析与摘要**：parseJTLResults聚合请求/事务样本、吞吐量、成功率、RT等指标；discoverExecutionResultPaths兼容本地与远程结果文件。

**章节来源**
- [execution.go:159-162](file://internal/service/execution.go#L159-L162)
- [execution.go:355-357](file://internal/service/execution.go#L355-L357)
- [execution.go:437-447](file://internal/service/execution.go#L437-L447)
- [execution.go:949-994](file://internal/service/execution.go#L949-L994)
- [execution.go:1044-1060](file://internal/service/execution.go#L1044-L1060)
- [execution.go:1062-1150](file://internal/service/execution.go#L1062-L1150)
- [execution.go:1432-1453](file://internal/service/execution.go#L1432-L1453)

### 关键流程时序图（代码级映射）
以下时序图对应CreateExecution主流程与异步执行细节，包括新增的CSV分片和错误明细捕获功能。

```mermaid
sequenceDiagram
participant H as "Handler"
participant S as "Service"
participant D as "DB"
participant P as "进程Map"
participant JM as "JMeter"
H->>S : CreateExecution(..., SaveHTTPDetails, IncludeMaster, SplitCSV)
S->>D : 查询脚本/在线Slave
D-->>S : 返回脚本与在线节点
S->>S : 创建执行记录/结果目录
S->>S : 检查CSV分片需求
S->>S : 拆分CSV文件并分发到Slave
S->>S : 动态计算JVM参数
S->>S : 构建本地/远程命令
S->>P : 注册进程组并启动goroutine
P-->>S : 并发执行本地/远程
S->>JM : 启动JMeter(带JVM参数)
JM-->>S : 写入日志与JTL
S->>S : 合并JTL/生成报告/解析摘要
S->>S : 处理失败请求明细捕获
S->>D : 更新状态/时长/摘要
S-->>H : 返回执行结果
```

**图表来源**
- [execution_handler.go:39-53](file://internal/handler/execution.go#L39-L53)
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [execution.go:368-463](file://internal/service/execution.go#L368-L463)

## 依赖分析
- **组件耦合**：
  - Handler依赖Service；Service依赖DB与Config；Slave服务独立维护节点状态；Script服务负责JMX文件管理。
  - **新增**：CSV拆分服务独立处理CSV文件的拆分与分发逻辑。
- **外部依赖**：
  - SQLite驱动（go-sqlite3）；Gin Web框架；JMeter可执行文件（路径来自配置）。
- **潜在循环依赖**：未发现直接循环；各层职责清晰。

```mermaid
graph LR
Handler["execution_handler.go"] --> Service["execution.go"]
Service --> DB["db.go"]
Service --> Config["config.go"]
Service --> ModelE["execution_model.go"]
SlaveSvc["slave_service.go"] --> DB
SlaveSvc --> ModelS["slave_model.go"]
ScriptSvc["script_service.go"] --> DB
CSVService["csv_split.go"] --> Service
```

**图表来源**
- [execution_handler.go:39-53](file://internal/handler/execution.go#L39-L53)
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [db.go:15-34](file://internal/database/db.go#L15-L34)
- [config.go:10-41](file://config/config.go#L10-L41)
- [slave_service.go:16-41](file://internal/service/slave.go#L16-L41)
- [script_service.go:18-83](file://internal/service/script.go#L18-L83)
- [csv_split.go:1-145](file://internal/service/csv_split.go#L1-L145)

**章节来源**
- [execution_handler.go:39-53](file://internal/handler/execution.go#L39-L53)
- [execution.go:104-481](file://internal/service/execution.go#L104-L481)
- [db.go:15-34](file://internal/database/db.go#L15-L34)
- [config.go:10-41](file://config/config.go#L10-L41)
- [slave_service.go:16-41](file://internal/service/slave.go#L16-L41)
- [script_service.go:18-83](file://internal/service/script.go#L18-L83)
- [csv_split.go:1-145](file://internal/service/csv_split.go#L1-L145)

## 性能考虑
- **并发执行**：本地与远程命令通过goroutine并发启动，提升整体吞吐；合并JTL与生成报告在完成后进行，避免I/O竞争。
- **内存参数**：动态计算JVM堆大小，避免固定值导致资源浪费或OOM；可通过JVM_ARGS覆盖。
- **I/O优化**：JTL合并与报告生成在本地磁盘完成，减少网络传输；日志统一写入execution.log便于实时监控。
- **资源探测**：优先使用/proc/meminfo，回退runtime统计，兼顾不同平台与容器环境。
- **CSV分片优化**：通过均匀拆分避免数据倾斜，提升分布式执行效率。
- **错误明细优化**：仅在失败时记录详细信息，减少正常执行的性能开销。

## 故障排查指南
- **"选中的Slave均不在线"**：检查Slave心跳与连通性；确认Slave状态为online后再发起执行。
- **"分布式保存HTTP明细需要先配置Master回调地址"**：在配置中设置JMeter.master_hostname，确保远程节点可回传错误明细。
- **"JMeter命令未生成"**：检查includeMaster与slaveIDs组合；确认JMeter路径与属性文件正确。
- **"执行长时间处于running"**：查看execution.log；确认JMeter进程是否正常；必要时调用停止接口。
- **"结果文件为空或未生成"**：确认JTL写入路径与权限；检查JMeter命令是否包含-l与-e/-o参数。
- **"CSV文件拆分失败"**：检查CSV文件是否存在；确认JMX中CSVDataSet配置正确。
- **"错误明细上传失败"**：检查上传令牌有效性；确认Master回调地址配置正确。

**章节来源**
- [execution.go:159-162](file://internal/service/execution.go#L159-L162)
- [execution.go:219-222](file://internal/service/execution.go#L219-L222)
- [execution.go:368-463](file://internal/service/execution.go#L368-L463)
- [execution_handler.go:137-151](file://internal/handler/execution.go#L137-L151)

## 结论
本系统通过清晰的分层设计与严谨的执行流程控制，实现了从脚本验证、Slave节点选择、JMeter命令构建、异步执行启动到状态更新的全链路自动化。**新增的三个核心功能**显著提升了系统的实用性和生产环境的可靠性：

- **失败请求明细捕获**：通过错误明细监听器和上传机制，实现了完整的HTTP请求/响应信息记录，极大提升了问题诊断能力
- **Master参与执行**：includeMaster参数提供了灵活的分布式+本地混合执行模式，便于统一分析和负载均衡
- **CSV数据分片**：splitCSV参数实现了大型数据集的自动拆分和分发，解决了分布式执行中的数据隔离问题

动态JVM内存计算提升了资源利用率；完善的错误处理与并发安全保证了生产环境的稳定性。建议在高并发场景下结合JVM_ARGS微调内存参数，并持续监控Slave连通性与执行日志，以获得最佳性能与可靠性。

## 附录
- **数据模型与接口**：
  - 执行记录模型：包含ID、脚本ID/名称、Slave IDs、状态、起止时间、时长、备注、结果/报告/日志路径、摘要等。
  - 响应模型：统一返回code/message/data结构，支持分页列表。
- **配置项要点**：
  - server.port：HTTP服务端口。
  - jmeter.path：JMeter可执行文件路径。
  - jmeter.master_hostname：RMI回调IP，多网卡时必填。
  - slave.heartbeat_interval：Slave心跳检测间隔（秒）。
  - dirs.data/uploads/results：数据、上传、结果目录。
  - jmeter.agent_csv_data_dir：Slave Agent的CSV数据目录。
- **新增执行选项**：
  - save_http_details：是否保存HTTP请求/响应详情
  - include_master：是否让Master参与分布式执行
  - split_csv：是否拆分CSV数据文件

**章节来源**
- [execution_model.go:3-18](file://internal/model/execution.go#L3-L18)
- [response_model.go:14-46](file://internal/model/response.go#L14-L46)
- [config.go:18-63](file://config/config.go#L18-L63)
- [execution_handler.go:29-37](file://internal/handler/execution.go#L29-L37)