...
## 架构总览
系统采用"微服务风格"的单体应用架构，后端以服务层为核心，通过处理器暴露 API，前端通过 HTTP 与 SSE 与后端交互。新增的CSV处理流程包括文件分割、分布式传输和自动清理机制。
...
DetectCSV --> CalcPartCount["计算分片数量(节点数±1)"]
...
UpdateStatus --> Cleanup["异步清理CSV文件"]
Cleanup --> End(["结束"])
```
...
- 配置模块独立于业务模块，便于部署与运维
- **CSV处理流程**：执行服务依赖CSV分割服务，CSV分割服务独立于其他模块
...
## 结论
本执行管理系统通过清晰的分层架构与完善的执行编排机制，实现了从脚本选择、节点分配到执行监控与结果收集的全流程自动化。**新增的CSV文件分割功能显著提升了系统处理大规模数据的能力**，通过智能分片、分布式传输和自动清理机制，确保了执行效率和资源利用率。系统具备良好的扩展性与可维护性，适合中小规模到中等规模的性能测试场景。建议在生产环境中结合监控与告警体系，进一步完善资源使用统计与容量规划。
