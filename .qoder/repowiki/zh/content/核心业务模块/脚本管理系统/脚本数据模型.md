...
## 更新摘要
**变更内容**
- 新增ScriptStats结构体，提供脚本和文件的统计信息
- 添加/get/api/scripts/stats API端点，支持实时统计数据查询
- 更新数据访问模式，包含统计查询功能
- 增强数据模型的监控和报表能力
...
## 架构概览
脚本数据模型的核心是"脚本"与"脚本文件"的一对多关系：一个脚本可拥有多个文件（主JMX文件与附属资源文件）。系统通过外键约束与级联删除策略确保数据一致性；通过索引提升查询性能；通过迁移机制平滑演进表结构。**新增**统计模块提供实时数据概览能力。
...
#### ScriptStats 结构体
- 字段
  - TotalScripts：脚本总数（int64）
  - TotalFiles：文件总数（int64）
- JSON标签
  - total_scripts：前端显示的脚本总数
  - total_files：前端显示的文件总数
- 业务含义
  - 提供系统级别的统计概览
  - 支持仪表板和监控界面的数据展示

**更新** 新增统计信息结构体，用于提供实时数据概览

**章节来源**
- [internal/model/script.go:43-46](file://internal/model/script.go#L43-L46)
...
#### 实时统计查询（新增）
- 查询逻辑
  - 执行 COUNT(*) FROM scripts 获取脚本总数
  - 执行 COUNT(*) FROM script_files 获取文件总数
  - 返回 ScriptStats 结构体
- API端点
...
**更新** 新增统计查询功能，提供实时数据概览

**章节来源**
- [internal/service/script.go:87-99](file://internal/service/script.go#L87-L99)
- [internal/handler/script.go:52-60](file://internal/handler/script.go#L52-L60)
- [internal/router/router.go:27](file://internal/router/router.go#L27)
...
ScriptStats --> Script : "统计脚本数量"
ScriptStats --> ScriptFile : "统计文件数量"
Service --> DB : "使用"
Service --> Script : "读写"
Service --> ScriptFile : "读写"
Service --> ScriptStats : "生成统计"
```
...
- **新增**：统计查询失败
  - 检查数据库连接状态
  - 确认 scripts 和 script_files 表存在且可访问
  - 验证COUNT查询权限
...
## 结论
脚本数据模型通过清晰的表结构、外键约束与索引设计，实现了脚本与其文件的一对多管理，并提供了完善的增删改查与分页聚合能力。**新增的统计模块**进一步增强了系统的监控和报表能力，通过轻量级的统计查询为前端仪表板提供实时数据概览。通过迁移机制与统一响应格式，系统具备良好的可维护性与扩展性。建议在高并发场景下进一步优化聚合统计与写入路径，并评估存储层升级以满足更大规模的业务需求。
...
- **新增**：统计一致性：统计查询基于原子性的COUNT操作，确保数据准确性

**更新** 新增统计一致性的数据完整性保障措施
