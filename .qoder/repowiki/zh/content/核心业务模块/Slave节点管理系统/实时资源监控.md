# 实时资源监控

<cite>
**本文档引用的文件**
- [main.go](file://main.go)
- [cmd/agent/main.go](file://cmd/agent/main.go)
- [internal/agent/server.go](file://internal/agent/server.go)
- [internal/router/router.go](file://internal/router/router.go)
- [config/config.go](file://config/config.go)
- [internal/handler/slave.go](file://internal/handler/slave.go)
- [internal/service/slave.go](file://internal/service/slave.go)
- [internal/model/slave.go](file://internal/model/slave.go)
- [internal/handler/execution.go](file://internal/handler/execution.go)
- [internal/service/execution.go](file://internal/service/execution.go)
- [internal/model/execution.go](file://internal/model/execution.go)
- [web/src/components/MetricTrendChart.vue](file://web/src/components/MetricTrendChart.vue)
- [web/src/api/execution.js](file://web/src/api/execution.js)
- [web/src/views/SlaveManage.vue](file://web/src/views/SlaveManage.vue)
- [web/src/api/slave.js](file://web/src/api/slave.js)
</cite>

## 更新摘要
**变更内容**
- 新增了Slave节点实时资源监控功能的完整实现
- 添加了CPU、内存、磁盘使用率的可视化展示和交互式进度条
- 更新了Slave管理界面的资源监控功能
- 完善了Agent代理的系统资源采集和展示功能

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构概览](#架构概览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考虑](#性能考虑)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)

## 简介

实时资源监控系统是一个基于JMeter的分布式性能测试管理系统，专注于提供全面的实时监控能力。该系统通过Agent代理收集Slave节点的系统资源信息，并通过Web界面实时展示这些监控数据。

系统的核心功能包括：
- **分布式资源监控**：通过Agent代理收集各Slave节点的CPU、内存、磁盘和网络连接状态
- **实时性能指标**：提供TPS、响应时间、成功率等关键性能指标的实时显示
- **心跳检测机制**：自动检测Slave节点的在线状态和健康状况
- **可视化图表**：通过SVG图表组件展示性能趋势和指标变化
- **交互式进度条**：提供直观的资源使用率可视化展示

## 项目结构

该项目采用Go语言开发，具有清晰的分层架构：

```mermaid
graph TB
subgraph "前端层 (Web)"
Vue[Vue.js 应用]
Components[组件库]
API[API封装]
SlaveManage[Slave管理界面]
end
subgraph "后端层 (Go)"
Main[主程序入口]
Router[路由管理]
Handler[处理器]
Service[业务逻辑]
Model[数据模型]
Agent[Agent代理]
end
subgraph "基础设施"
Config[配置管理]
Database[(SQLite数据库)]
JMeter[JMeter引擎]
end
Vue --> API
API --> Router
Router --> Handler
Handler --> Service
Service --> Database
Service --> Agent
Agent --> JMeter
SlaveManage --> Vue
```

**图表来源**
- [main.go:28-66](file://main.go#L28-L66)
- [internal/router/router.go:14-117](file://internal/router/router.go#L14-L117)

**章节来源**
- [main.go:1-83](file://main.go#L1-L83)
- [internal/router/router.go:1-134](file://internal/router/router.go#L1-L134)

## 核心组件

### Agent代理服务

Agent代理是实时监控系统的关键组件，负责在各个Slave节点上收集系统资源信息。

```mermaid
classDiagram
class Server {
+string dataDir
+string token
+ServeMux mux
+NewServer(dataDir, token) Server
+setupRoutes() void
+Start(addr) error
+handleHealth(w, r) void
+handleUpload(w, r) void
+handleFileOperations(w, r) void
}
class SystemStats {
+CPUStats cpu
+MemoryStats memory
+DiskStats disk
+NetworkStats network
}
class CPUStats {
+float64 percent
+int count
}
class MemoryStats {
+uint64 total_mb
+uint64 used_mb
+float64 percent
}
class DiskStats {
+uint64 total_mb
+uint64 used_mb
+float64 percent
}
class NetworkStats {
+int connections
}
Server --> SystemStats : "收集"
SystemStats --> CPUStats : "包含"
SystemStats --> MemoryStats : "包含"
SystemStats --> DiskStats : "包含"
SystemStats --> NetworkStats : "包含"
```

**图表来源**
- [internal/agent/server.go:89-113](file://internal/agent/server.go#L89-L113)
- [internal/agent/server.go:25-51](file://internal/agent/server.go#L25-L51)

### 心跳检测系统

系统实现了自动化的Slave节点心跳检测机制：

```mermaid
sequenceDiagram
participant Main as 主程序
participant Heartbeat as 心跳检测器
participant Slave as Slave节点
participant Agent as Agent代理
participant DB as 数据库
Main->>Heartbeat : StartHeartbeat(interval)
loop 每个检测周期
Heartbeat->>Slave : 检测JMeter RMI端口
Slave-->>Heartbeat : 连接状态
Heartbeat->>Agent : GET /health
Agent-->>Heartbeat : 系统状态
Heartbeat->>DB : 更新节点状态
end
```

**图表来源**
- [internal/service/slave.go:448-459](file://internal/service/slave.go#L448-L459)
- [internal/service/slave.go:461-523](file://internal/service/slave.go#L461-L523)

### 实时资源监控界面

系统提供了完整的Slave节点资源监控界面，包括交互式进度条展示：

```mermaid
classDiagram
class SlaveManage {
+Array slaveList
+Object currentSlaveStats
+Boolean statsDialogVisible
+parseSystemStats(slave) Object
+getResourceColor(percent, threshold) string
+showStatsDialog(row) void
+loadSlaves() void
}
class ResourceCell {
+Object parsedStats
+Array resourceRows
+renderProgressBars() void
}
class StatsDialog {
+Object currentSlaveStats
+Number currentSlaveUptime
+renderDetailedStats() void
}
SlaveManage --> ResourceCell : "管理"
SlaveManage --> StatsDialog : "显示"
ResourceCell --> StatsDialog : "触发"
```

**图表来源**
- [web/src/views/SlaveManage.vue:131-170](file://web/src/views/SlaveManage.vue#L131-L170)
- [web/src/views/SlaveManage.vue:275-326](file://web/src/views/SlaveManage.vue#L275-L326)

**章节来源**
- [internal/agent/server.go:1-326](file://internal/agent/server.go#L1-L326)
- [internal/service/slave.go:1-542](file://internal/service/slave.go#L1-L542)
- [web/src/views/SlaveManage.vue:1-1512](file://web/src/views/SlaveManage.vue#L1-L1512)

## 架构概览

系统采用分层架构设计，实现了前后端分离和职责明确的模块划分：

```mermaid
graph TB
subgraph "用户界面层"
UI[Web界面]
SlaveManage[Slave管理界面]
Charts[图表组件]
Controls[控制面板]
end
subgraph "API网关层"
Router[路由处理]
Middleware[中间件]
end
subgraph "业务逻辑层"
ExecutionSvc[执行服务]
SlaveSvc[Slave服务]
ConfigSvc[配置服务]
end
subgraph "数据访问层"
DB[(SQLite数据库)]
FS[(文件系统)]
end
subgraph "外部系统"
JMeter[JMeter引擎]
Agents[Agent代理集群]
end
UI --> Router
SlaveManage --> Router
Charts --> Router
Controls --> Router
Router --> ExecutionSvc
Router --> SlaveSvc
Router --> ConfigSvc
ExecutionSvc --> DB
ExecutionSvc --> FS
SlaveSvc --> DB
SlaveSvc --> Agents
ExecutionSvc --> JMeter
```

**图表来源**
- [internal/router/router.go:14-117](file://internal/router/router.go#L14-L117)
- [main.go:28-66](file://main.go#L28-L66)

## 详细组件分析

### 实时性能指标系统

系统通过分析JTL结果文件实时计算各种性能指标：

```mermaid
flowchart TD
Start([开始监控]) --> ReadJTL[读取JTL文件]
ReadJTL --> ParseCSV[解析CSV格式]
ParseCSV --> ExtractFields[提取关键字段]
ExtractFields --> BucketData[按秒分桶聚合]
BucketData --> CalcMetrics[计算性能指标]
CalcMetrics --> TPS[计算TPS]
CalcMetrics --> RT[计算响应时间]
CalcMetrics --> SuccessRate[计算成功率]
CalcMetrics --> Concurrency[计算并发数]
TPS --> StoreData[存储指标数据]
RT --> StoreData
SuccessRate --> StoreData
Concurrency --> StoreData
StoreData --> ChartUpdate[更新图表显示]
ChartUpdate --> End([结束])
```

**图表来源**
- [internal/service/execution.go:882-1010](file://internal/service/execution.go#L882-L1010)
- [internal/service/execution.go:1115-1129](file://internal/service/execution.go#L1115-L1129)

### 资源监控数据模型

系统定义了完整的资源监控数据结构：

| 组件 | 字段 | 类型 | 描述 |
|------|------|------|------|
| SystemStats | cpu | CPUStats | CPU使用率和核心数 |
| SystemStats | memory | MemoryStats | 内存总量和使用情况 |
| SystemStats | disk | DiskStats | 磁盘空间使用情况 |
| SystemStats | network | NetworkStats | 网络连接数 |
| CPUStats | percent | float64 | CPU使用百分比 |
| CPUStats | count | int | CPU核心数 |
| MemoryStats | total_mb | uint64 | 总内存(MB) |
| MemoryStats | used_mb | uint64 | 已用内存(MB) |
| MemoryStats | percent | float64 | 内存使用百分比 |
| DiskStats | total_mb | uint64 | 总磁盘(MB) |
| DiskStats | used_mb | uint64 | 已用磁盘(MB) |
| DiskStats | percent | float64 | 磁盘使用百分比 |
| NetworkStats | connections | int | 网络连接数 |

**章节来源**
- [internal/agent/server.go:25-51](file://internal/agent/server.go#L25-L51)
- [internal/model/slave.go:3-30](file://internal/model/slave.go#L3-L30)

### 前端可视化组件

前端使用Vue.js构建了丰富的可视化组件，包括交互式进度条：

```mermaid
classDiagram
class MetricTrendChart {
+string title
+mixed value
+string unit
+Array points
+string field
+string color
+number height
+Array chartData
+Array chartPoints
+computedValue() string
+formatMetricValue(value) string
+handleHover(event) void
}
class LiveMetricPoint {
+string timestamp
+number epoch_second
+number tps
+number request_rate
+number avg_rt
+number success_rate
+number error_rate
+number concurrency
+number total_requests
+number p95_rt
+number p99_rt
+number error_count
+number bytes_per_sec
}
class ResourceCell {
+Object parsedStats
+Array resourceRows
+renderProgressBars() void
}
class StatsDialog {
+Object currentSlaveStats
+Number currentSlaveUptime
+renderDetailedStats() void
}
MetricTrendChart --> LiveMetricPoint : "渲染"
ResourceCell --> StatsDialog : "触发"
```

**图表来源**
- [web/src/components/MetricTrendChart.vue:142-341](file://web/src/components/MetricTrendChart.vue#L142-L341)
- [web/src/views/SlaveManage.vue:131-170](file://web/src/views/SlaveManage.vue#L131-L170)
- [web/src/views/SlaveManage.vue:275-326](file://web/src/views/SlaveManage.vue#L275-L326)

**章节来源**
- [web/src/components/MetricTrendChart.vue:1-526](file://web/src/components/MetricTrendChart.vue#L1-L526)
- [web/src/api/execution.js:1-88](file://web/src/api/execution.js#L1-L88)
- [web/src/views/SlaveManage.vue:1-1512](file://web/src/views/SlaveManage.vue#L1-L1512)

### 实时资源监控流程

系统实现了完整的实时资源监控流程：

```mermaid
sequenceDiagram
participant UI as 用户界面
participant API as API接口
participant Service as 业务服务
participant Agent as Agent代理
participant DB as 数据库
UI->>API : 请求Slave列表
API->>Service : 获取Slave列表
Service->>DB : 查询数据库
DB-->>Service : 返回Slave数据
Service->>Agent : 调用/health接口
Agent-->>Service : 返回系统资源数据
Service->>DB : 更新system_stats字段
DB-->>Service : 更新成功
Service-->>API : 返回完整数据
API-->>UI : 展示资源监控界面
UI->>API : 定时刷新心跳状态
API->>Service : 获取心跳状态
Service->>DB : 查询最新状态
Service->>Agent : 批量检测Agent状态
Agent-->>Service : 返回状态数据
Service->>DB : 更新状态和资源数据
DB-->>Service : 更新成功
Service-->>API : 返回心跳数据
API-->>UI : 更新界面显示
```

**图表来源**
- [web/src/views/SlaveManage.vue:806-844](file://web/src/views/SlaveManage.vue#L806-L844)
- [internal/service/slave.go:466-542](file://internal/service/slave.go#L466-L542)

**章节来源**
- [web/src/views/SlaveManage.vue:806-844](file://web/src/views/SlaveManage.vue#L806-L844)
- [internal/service/slave.go:466-542](file://internal/service/slave.go#L466-L542)

## 依赖关系分析

系统的主要依赖关系如下：

```mermaid
graph LR
subgraph "外部依赖"
gopsutil[gopsutil v4]
gin[gin-gonic/gin]
yaml[gopkg.in/yaml.v3]
sqlite[sqlite3]
elementplus[element-plus]
vue[Vue.js]
end
subgraph "内部模块"
main[main.go]
agent[internal/agent]
router[internal/router]
handler[internal/handler]
service[internal/service]
model[internal/model]
config[config]
end
main --> router
router --> handler
handler --> service
service --> model
service --> config
agent --> gopsutil
router --> gin
config --> yaml
service --> sqlite
```

**图表来源**
- [internal/agent/server.go:3-19](file://internal/agent/server.go#L3-L19)
- [internal/router/router.go:3-12](file://internal/router/router.go#L3-L12)

**章节来源**
- [go.mod](file://go.mod)
- [go.sum](file://go.sum)

## 性能考虑

### 并发处理优化

系统在多个层面实现了性能优化：

1. **心跳检测并发控制**：使用信号量限制同时检测的Slave节点数量
2. **CSV文件处理**：采用流式读取避免大文件内存占用
3. **数据库连接池**：合理配置数据库连接数
4. **缓存策略**：对频繁访问的数据进行缓存
5. **前端组件优化**：使用虚拟滚动和懒加载减少DOM压力

### 内存管理

- **JVM参数动态计算**：根据系统可用内存动态调整JMeter JVM参数
- **文件处理优化**：使用bufio.Scanner进行高效文件读取
- **数据结构优化**：使用sync.Map提高并发访问性能
- **资源数据压缩**：前端对system_stats进行JSON解析和缓存

### 网络通信优化

- **HTTP客户端复用**：避免重复创建HTTP连接
- **超时控制**：合理设置网络操作超时时间
- **错误重试机制**：对临时性网络错误进行自动重试
- **批量检测**：心跳检测支持并发批量处理

### 前端性能优化

- **组件懒加载**：按需加载复杂的图表组件
- **虚拟滚动**：大量数据时使用虚拟滚动
- **防抖节流**：对高频操作进行防抖处理
- **状态缓存**：缓存计算结果避免重复计算

## 故障排除指南

### 常见问题及解决方案

| 问题类型 | 症状 | 可能原因 | 解决方案 |
|----------|------|----------|----------|
| Agent无法连接 | /health返回401 | Token配置错误 | 检查Agent配置中的token |
| Slave节点离线 | 心跳检测显示offline | 网络连接问题 | 检查Slave节点网络连通性 |
| 性能指标不更新 | 图表无数据变化 | JTL文件读取失败 | 检查JTL文件权限和路径 |
| 内存不足 | 系统响应缓慢 | JVM内存配置过低 | 调整JVM参数配置 |
| 资源监控异常 | 进度条显示异常 | Agent资源采集失败 | 检查Agent进程和权限 |
| 前端渲染卡顿 | 页面加载缓慢 | 数据量过大 | 优化数据过滤和分页 |

### 调试工具

系统提供了多种调试和监控工具：

1. **健康检查端点**：`/api/slaves/:id/check` 获取详细的诊断信息
2. **日志流**：SSE方式实时输出执行日志
3. **系统状态**：Agent的`/health`端点提供系统资源状态
4. **配置验证**：检查配置文件格式和参数有效性
5. **资源监控**：Slave管理界面的实时资源展示功能

**章节来源**
- [internal/handler/slave.go:111-144](file://internal/handler/slave.go#L111-L144)
- [internal/service/slave.go:195-247](file://internal/service/slave.go#L195-L247)

## 结论

实时资源监控系统通过精心设计的架构和实现，为JMeter分布式性能测试提供了全面的监控能力。系统的主要优势包括：

1. **全面的监控覆盖**：从系统资源到应用性能的多层次监控
2. **实时性保障**：通过Agent代理和SSE技术实现实时数据更新
3. **可视化展示**：丰富的图表组件提供直观的数据展示
4. **交互式体验**：进度条和弹窗详情提供良好的用户体验
5. **高可用性**：心跳检测和故障转移机制确保系统稳定性
6. **易扩展性**：模块化设计便于功能扩展和维护

该系统为性能测试团队提供了强大的工具，能够有效监控分布式环境下的系统状态，及时发现和解决性能问题，提高测试效率和质量。新增的Slave节点实时资源监控功能进一步增强了系统的实用性，为用户提供了更加直观和全面的监控体验。