...
## 更新摘要
**变更内容**
- 故障排除指南现在通过README.md的常见问题章节提供，不再单独维护独立的故障排除文档
- 常见问题章节涵盖编译错误、网络连接问题、性能问题等核心故障排除场景
- 故障排除策略与环境管理策略整合到README.md中，便于维护和更新
...
## 简介
本指南面向运维与开发人员，系统化梳理 JMeter Admin 在编译、运行、网络、性能等方面的常见问题与排障流程。**重要更新**：故障排除指南现已整合到README.md的常见问题章节中，提供编译错误、网络连接问题、性能问题等核心故障排除场景的解决方案。文档覆盖开发、测试、生产三类环境的差异化策略，以及紧急事件的处置预案与恢复措施。同时给出版本升级与迁移过程中的注意事项与解决方案。
...
### 一、编译错误
**重要更新**：编译错误相关问题已在README.md的常见问题章节中提供详细解答。

- CGO/SQLite 缺失 gcc
  - 现象：编译时报 CGO 相关错误。
  - 排查：确认系统已安装 gcc 与 build-essential。
  - 解决：参考 README.md 常见问题章节中的 GCC 安装指导。
  - 参考
...
- Node.js 版本过低
  ...
  - 解决：使用 README.md 中的一键安装脚本或升级 Node.js。
  - 参考
...
- Go 版本不匹配
  ...
  - 解决：使用 README.md 中的一键安装脚本安装推荐版本。
  - 参考
...
- 前端构建缓慢
  ...
  - 排查：网络环境与镜像源。
  - 解决：使用 README.md 中的 npm 镜像配置或参考加速建议。
  - 参考
...
### 二、运行时错误
**重要更新**：运行时错误相关问题已在README.md的常见问题章节中提供详细解答。

- 服务启动失败
  - 现象：启动后立即退出或端口占用。
  - 排查：查看日志文件、检查端口占用、确认配置文件。
  - 解决：使用 README.md 中的部署脚本 start/stop/restart，或手动检查 PID/日志。
  - 参考
...
### 三、网络连接问题
**重要更新**：网络连接问题相关问题已在README.md的常见问题章节中提供详细解答。
...
### 四、性能问题
**重要更新**：性能问题相关问题已在README.md的常见问题章节中提供详细解答。
...
### 九、版本升级与迁移
**重要更新**：版本升级与迁移策略已在README.md中提供统一指导。
...
### 十、社区支持与问题反馈
**重要更新**：社区支持与问题反馈渠道已在README.md中提供统一指导。
...
## 结论
JMeter Admin 通过"嵌入式前端 + Gin API + SQLite"的架构实现了轻量、易部署的分布式压测管理平台。**重要更新**：故障排除指南现已整合到README.md的常见问题章节中，提供更加集中和易于维护的故障排除指导。针对编译、运行、网络、性能等常见问题，建议遵循本文提供的系统化排障流程：先检查环境与配置，再结合日志与实时指标定位问题，最后通过数据库迁移与服务治理完成恢复。在不同环境中采用差异化的策略与工具链，可显著提升稳定性与可维护性。
