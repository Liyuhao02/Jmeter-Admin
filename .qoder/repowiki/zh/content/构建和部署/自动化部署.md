...
- [scripts/server-deploy.sh](file://scripts/server-deploy.sh)
- [scripts/server-deploy.env.example](file://scripts/server-deploy.env.example)
- [scripts/clean-local-artifacts.sh](file://scripts/clean-local-artifacts.sh)
...
## 更新摘要
**变更内容**
- 新增完整的服务器部署脚本系统，包含 master/agent/all 三种部署模式
- 新增环境配置模板和自动化依赖安装功能
- 新增 systemd 服务管理支持
- 新增生产环境部署指导和最佳实践
- 新增日志管理和状态监控功能
...
SS[scripts/server-deploy.sh<br/>服务器部署脚本]
ENV[scripts/server-deploy.env.example<br/>环境配置模板]
CLEAN[scripts/clean-local-artifacts.sh<br/>清理脚本]
MK[Makefile<br/>构建命令]
END
M --> CFG
M --> DB
...
- [scripts/server-deploy.sh:1-442](file://scripts/server-deploy.sh#L1-L442)
- [scripts/server-deploy.env.example:1-12](file://scripts/server-deploy.env.example#L1-L12)
...
# 2. 安装所有依赖（Go、Node.js、gcc、Java、JMeter）
...
```bash
# 部署脚本支持的操作
./scripts/server-deploy.sh <role> <action>
...
**配置文件位置**
- 主配置文件：`config.yaml`（自动生成）
- 服务器配置：`scripts/server-deploy.env`（手动配置）
...
MASTER_PORT=8080         # master 端口
AGENT_PORT=8089          # agent 端口
AGENT_DATA_DIR=/opt/jmeter/csv-data  # CSV 数据目录
AGENT_TOKEN=             # 认证 token
AGENT_JMETER_PATH=/opt/apache-jmeter-5.6.3/bin/jmeter  # JMeter 路径
```
...
sudo systemctl stop jmeter-admin  
sudo systemctl restart jmeter-admin
...
```bash
# 查看 master 状态
./scripts/server-deploy.sh master status

# 查看 agent 状态  
./scripts/server-deploy.sh agent status
...
# 实时查看 agent 日志
./scripts/server-deploy.sh agent logs

# 实时查看所有日志
./scripts/server-deploy.sh all logs
```
...
**清理范围**
- Go 缓存：`.gocache`
- 构建产物：`web/dist`, `jmeter-admin`, `jmeter-agent`
- 依赖包：`web/node_modules`
- 日志文件：`*.log`, `*.jtl`, `*.csv`
- 数据库文件：`*.db`, `*.sqlite`
...
- [scripts/server-deploy.sh:1-442](file://scripts/server-deploy.sh#L1-L442)
- [scripts/server-deploy.env.example:1-12](file://scripts/server-deploy.env.example#L1-L12)
- [scripts/clean-local-artifacts.sh:1-44](file://scripts/clean-local-artifacts.sh#L1-L44)
- [deploy.sh:1-537](file://deploy.sh#L1-L537)